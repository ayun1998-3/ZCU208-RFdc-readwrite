...
#define MAX_PKT_LEN		0x200 // 512 in decimal
...
#endif
{
	...
	TxBufferPtr = (s16 *)TX_BUFFER_BASE ;
	RxBufferPtr = (s16 *)RX_BUFFER_BASE;
...
//	Value = TEST_START_VALUE;
	...
	// 	TxBufferPtr[Index] = (u8) Value & 0xFF;
    //     TxBufferPtr[Index + 1] = (u8) (Value & 0xFF00) >> 2;        
    ...
    for (int i = 0; i < MAX_PKT_LEN; i++) {
        ...
        // xil_printf("Phase: %d  ", phase);   
        // double value =  round(sin(phase) * 0x400);
        // int whole, thousandths;
        // whole = value;
        // thousandths = (value - whole) * 1000;
        // xil_printf("Value: %d.%3d\r\n", whole, thousandths);        
        s16 pre_MSB_aligned = (s16) round(sin(phase) * amplitude);
        TxBufferPtr[i] = (pre_MSB_aligned & 0x3FFF) << 2;
        phase += 2 * M_PI / 90.0;                
    }
	 ...
	 * is enabled
	 */
	Xil_DCacheFlushRange((UINTPTR)TxBufferPtr, (MAX_PKT_LEN) * 2);
	Xil_DCacheFlushRange((UINTPTR)RxBufferPtr, (MAX_PKT_LEN) * 2);
	...
	for (Index = 0; Index < Tries; Index ++) {
        ...
        Status = XAxiDma_SimpleTransfer(&AxiDma, (UINTPTR) TxBufferPtr,
						(MAX_PKT_LEN * 2), XAXIDMA_DMA_TO_DEVICE);
        // length is the number of 8-bit values to transfer. We're transmitting 16-bit values, so MAX_PKT_LEN * 2 total values.
		...
		Status = XAxiDma_SimpleTransfer(&AxiDma, (UINTPTR) RxBufferPtr,
						(MAX_PKT_LEN * 2), XAXIDMA_DEVICE_TO_DMA);  // the receive transaction starts after a delay
...
static int CheckData(void)
{
	...
	for (Index = 0; Index < MAX_PKT_LEN; Index++) {
        ...
        // xil_printf("Data index %d: received %x when %x was sent\r\n",
