...
int ClockedExample(u16 RFdcDeviceId)
{
    ...
    XRFdc_Distribution_Settings got_Distribution_Settings;
    u8 GetFIFOEnable;
    u8 SetFIFOEnable;
    ...
    Status = XRFdc_IntrEnable(RFdcInstPtr, XRFDC_ADC_TILE, Tile, Block, XRFDC_EN_INTR_ADC_TILE0_MASK);    
    ...
    // check if clock distribution has been set correctly
    Status = XRFdc_SetClkDistribution(RFdcInstPtr, &got_Distribution_Settings);    
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 4; j++) {
            if (i == 0) {
                printf("Sample rate for ADC tile %d: %f", j, got_Distribution_Settings.SampleRates[i][j]);
            }
            else {
                printf("Sample rate for DAC tile %d: %f", j, got_Distribution_Settings.SampleRates[i][j]);
            }
        }
    }

    // disable FIFOs and check if they're disabled
    SetFIFOEnable = 0x0;
